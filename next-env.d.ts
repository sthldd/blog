...
declare module 'cos-nodejs-sdk-v5' {
    ...
    export interface SliceUploadFileOptions {
      ...
      StorageClass?: string
      onTaskReady?: (taskId: string) => void
      onHashProgress?: (progressData: {loaded: number; total: number; speed: number; percent: number}) => void
      onProgress?: (progressData: {loaded: number; total: number; speed: number; percent: number}) => void
    }
    ...
    declare class COS {
      ...
      // 列出存储桶列表
      public getService(cb: (err: COSError | null, data: BucketListResult) => void): void
      // 列出指定地域的存储桶列表。
      public getService(options: BucketListOptions, cb: (err: COSError | null, data: BucketListResult) => void): void
      // PUT Bucket 接口请求可以在指定账号下创建一个存储桶。
      public putBucket(options: CreateBucketOptions, cb: (err: COSError | null, data: CreateBucketResult) => void): void
      // 检索存储桶及其权限
      public headBucket(options: BucketOptions, cb: (err: COSError | null, data: COSResult) => void): void
      // 删除存储桶
      public deleteBucket(options: BucketOptions, cb: (err: COSError | null, data: COSResult) => void): void
      // 设置存储桶 ACL
      public putBucketAcl(options: BucketACLOptions, cb: (err: COSError | null, data: COSResult) => void): void
      // 查询存储桶 ACL
      public getBucketAcl(options: BucketOptions, cb: (err: COSError | null, data: BucketACLDetailResult) => void): void
      // 查询对象列表
      public getBucket(
        options: BucketContentsOptions,
        cb: (err: COSError | null, data: BucketContentsResult) => void
      ): void
      // 简单上传对象
      public putObject(
        options: UploadBucketObjectOptions,
        cb: (err: COSError | null, data: UploadBucketObjectResult) => void
      ): void
      // 下载对象
      public getObject(
        options: DownloadObjectOptions,
        cb: (err: COSError | null, data: DownloadObjectResult) => void
      ): void
      // 复制对象
      public putObjectCopy(options: CopyObjectOptions, cb: (err: COSError | null, data: CopyObjectResult) => void): void
      // 预请求跨域配置
      public optionsObject(
        options: ObjectCrossDomainOptions,
        cb: (err: COSError | null, data: ObjectCrossDomainResult) => void
      ): void
      // 删除单个对象
      public deleteObject(options: DeleteObjectOptions, cb: (err: COSError | null, data: COSResult) => void): void
      // 删除多个对象
      public deleteMultipleObject(
        options: DeleteMultipleObjectOptions,
        cb: (err: COSError | null, data: DeleteMultipleObjectResult) => void
      ): void
      // 分块上传对象
      public sliceUploadFile(
        options: SliceUploadFileOptions,
        cb: (err: COSError | null, data: SliceUploadFileResult) => void
      ): void
      // 复制对象 用于大文件
      public sliceCopyFile(
        options: SliceCopyFileOptions,
        cb: (err: COSError | null, data: SliceCopyFileResult) => void
      ): void
      // 批量上传
      public uploadFiles(
        options: MultiFileUploadOptions,
        cb: (err: COSError | null, data: MultiFileUploadResult) => void
      ): void
      // 获取任务列表
